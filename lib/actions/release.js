...
const releaseAction = async (options) => {
  ...
  gitClient
    ...
    .addIgnoreFile('.npmrc')
    .addIgnoreFile('yarn-lock')
    .addIgnoreFile('yarn-error.log')
  ...
  // console.log('ReleaseOptions', releaseOptions)
...
//     }
  ...
  if (releaseOptions.merge) {
    await gitClient.checkout(releaseOptions.mergeBranch)
    await gitClient.pull()
    await gitClient.merge(releaseOptions.branch)
    await gitClient.add('.')
    await gitClient.commit(`${bumped.version} published ***NO_CI***`)
    await gitClient.push(gitClient.remote, `HEAD:${releaseOptions.mergeBranch}`)
  }
}
