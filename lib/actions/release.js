...
const releaseAction = async (options) => {
  ...
  if (releaseOptions.commit) {
    try {
      ...
      // await shell.exec(`git commit -a -m "Bumped version to ${bumped.version}"`)

      const getPushCommand = `git push`
      // logger(getPushCommand)
      await gitClient.client.raw(getPushCommand)
    } catch (err) {
